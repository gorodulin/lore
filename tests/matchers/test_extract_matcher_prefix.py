...
def test_glob_with_fixed_prefix():
    assert extract_matcher_prefix("p:src/api/**/*.ts") == ["src", "api"]


def test_glob_starting_with_globstar():
    assert extract_matcher_prefix("p:**/*.py") == []


def test_simple_glob():
    assert extract_matcher_prefix("p:src/*.py") == ["src"]


def test_regex_returns_none():
    assert extract_matcher_prefix("c:import os") is None
