...
def test_load_single_rules_file(tmp_path):
    _, fact = build_test_fact(incl=["g:*.ts"])
    ...
    assert results[0]["facts"]["1"]["incl"] == ["g:*.ts"]
...
def test_load_nested_rules_files(tmp_path):
    ...
    assert root_rules["facts"]["1"]["incl"] == ["g:*.ts"]
    assert app_rules["facts"]["2"]["incl"] == ["g:**/api.js"]


def test_load_deeply_nested(tmp_path):
    deep_dir = tmp_path / "a" / "b" / "c"
    deep_dir.mkdir(parents=True)
    _, fact = build_test_fact(incl=["g:*.js"])
