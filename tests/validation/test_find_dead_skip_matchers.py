...
class TestFindDeadNegatives:
    ...
    def test_valid_exclusion_overlaps_with_incl(self):
        fact = {
            "fact": "Test",
            "incl": ["p:src/**/*.js"],
            "skip": ["p:src/vendor/**"],  # overlaps with src/
        }
        result = find_dead_skip_matchers(fact)
        assert result == []
    ...
    def test_dead_exclusion_no_overlap(self):
        fact = {
            "fact": "Test",
            "incl": ["p:src/**/*.js"],
            "skip": ["p:vendor/**"],  # doesn't overlap with src/
        }
        result = find_dead_skip_matchers(fact)
        assert result == ["p:vendor/**"]

    def test_multiple_dead_exclusions(self):
        fact = {
            "fact": "Test",
            "incl": ["p:src/**/*.js"],
            "skip": ["p:vendor/**", "p:lib/**"],  # neither overlaps
        }
        result = find_dead_skip_matchers(fact)
        assert set(result) == {"p:vendor/**", "p:lib/**"}

    def test_mixed_valid_and_dead(self):
        fact = {
            "fact": "Test",
            "incl": ["p:src/**/*.js"],
            "skip": ["p:src/test/**", "p:vendor/**"],  # first valid, second dead
        }
        result = find_dead_skip_matchers(fact)
        assert result == ["p:vendor/**"]
    ...
    def test_multiple_incl_patterns(self):
        fact = {
            "fact": "Test",
            "incl": ["p:src/**/*.js", "p:lib/**/*.js"],
            "skip": ["p:src/vendor/**"],  # overlaps with first incl
        }
        result = find_dead_skip_matchers(fact)
        assert result == []
    ...
    def test_dead_skip_no_incl_overlap(self):
        fact = {
            "fact": "Test",
            "incl": ["p:src/**/*.js", "p:lib/**/*.js"],
            "skip": ["p:vendor/**"],  # doesn't overlap with either
        }
        result = find_dead_skip_matchers(fact)
        assert result == ["p:vendor/**"]
    ...
    def test_non_glob_matcher_ignored(self):
        fact = {
            "fact": "Test",
            "incl": ["p:src/**/*.js"],
            "skip": ["c:vendor/.*"],  # regex, can't analyze
        }
        result = find_dead_skip_matchers(fact)
