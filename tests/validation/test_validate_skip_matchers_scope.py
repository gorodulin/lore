...
class TestValidateSkipPatternsScope:
    ...
    def test_multiple_skip_patterns_mixed(self):
        ...
        result = validate_skip_matchers_scope(
            incl=["g:src/**/*.ts"],
            skip=[
                "g:src/**/*.test.ts",      # Valid - within scope
                "g:vendor/**",              # Invalid - outside scope
                "g:**/node_modules/**",     # Valid - has globstar
                "g:lib/**",                 # Invalid - outside scope
            ],
        )
        assert len(result) == 2
        assert result[0][0] == "g:vendor/**"
        assert result[1][0] == "g:lib/**"
